...
cdef class Malandro:
    ...
    def runAnnealling(self, stepsPerTemperature=10000,
        ...
        if relevantSpinSystems:
            ...
            # todo, more explanation to user here.
            self.notifyTextObservers('Nothing to do.')
    ...
    def leaveOutSubSetOfPeaks(self, fraction):
        ...
        if fraction == 0.0:
            ...
            for spectrum in spectra:
                peaksToLeaveOutList.extend(
                    spectrum.getRandomSubSetofPeaks(fraction))
    ...
    cdef void calculateAllPeakContributions(self):
        ...
        for spectrum in self.dataModel.spectra:
            ...
            spectrum.findPossiblePeakContributions(
                self.useDimensionalAssignments)
    ...
    cdef void annealingSub(self, double AcceptanceConstant,
        ...
        for aTry in xrange(stepsPerTemperature):
            ...
            # Both spin systems are assigned to a residue
            if not currentResidueA is None and not currentResidueB is None:
                ...
                # A and B happen to be a sequential pair AB
                if currentResidueA is previousResB:
                ...
                # sequential pair BA
                elif currentResidueB is previousResA:
                ...
                # A and B are not sequential
                    ...
                    newPeaks = l5.activePeakLinks + l6.activePeakLinks + \
                        l7.activePeakLinks + l8.activePeakLinks
                    peakSet = oldPeaks + newPeaks

            # spin system B is not assigned to any residue
            elif not currentResidueA is None:
            ...
            # spin system A is not assigned to any residue
            elif not currentResidueB is None:
