...
import AutoAssign.src.python.assignMentTransferTab
reload(AutoAssign.src.python.assignMentTransferTab)
from AutoAssign.src.python.assignMentTransferTab import AssignMentTransferTab
import AutoAssign.src.python.spectrumSelectionTab
reload(AutoAssign.src.python.spectrumSelectionTab)
from AutoAssign.src.python.spectrumSelectionTab import SpectrumSelectionTab
import AutoAssign.src.python.saveLoadTab
reload(AutoAssign.src.python.saveLoadTab)
from AutoAssign.src.python.saveLoadTab import SaveLoadTab
import AutoAssign.src.python.annealingSettingsTab
reload(AutoAssign.src.python.annealingSettingsTab)
from AutoAssign.src.python.annealingSettingsTab import AnnealingSettingsTab
import AutoAssign.src.python.resultsTab
reload(AutoAssign.src.python.resultsTab)
from AutoAssign.src.python.resultsTab import ResultsTab
#import benchMark
# reload(benchMark)
    ...
    def checkSpectraAndLabelling(self):
        ...
        for spec in self.selectedSpectra:
            ...
            if not spec.ccpnSpectrum.experiment.refExperiment:
                ...
                showWarning(
                    'No Reference Experiment', string, parent=self.GUI)
            ...
            if spec.labellingScheme is True and not mixtures:
                ...
                string = name + ''' is not connected to any labelled sample.
                                    In order to determine the labelling scheme
                                    automatically from the experiment,
                                    this has to be set. Go to
    ...
    def loadDataFromPyc(self, fileName):
        ...
        if fileExists:
            ...
            try:
                ...
                string = '''Probably the file you try to
            ...
            if results:
                ...
                if isinstance(results, myDataModel):
                    ...
                    self.results.connectToProject(
                        self.project, self.nmrProject)
                    # TODO: this is not very nice
                    self.GUI.resultsTab.dataModel = self.results
...
class ViewAssignmentPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(guiFrame, options=['Spectra',
                                                     'Annealing',
                                                     'Results',
        ...
        bottomButtons = UtilityButtonList(tabbedFrame.sideFrame, helpUrl='www.google.com')
