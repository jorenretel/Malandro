...
cdef class Residue:
    '''Residue, is used as a wrapper around the ccpn residue object.
      ...
      ccpCode:            three-letter code indicating the amino acid
                          type of the residue, 'Asp' for instance.
      ...
      linkDict:           dict that has the link objects that describes
                          ...
                          Keys to this dict are a combination of the
                          serials of these two spin-systems. The reason
                          that this 'matrix' is an instance variable
                          instead of having one global matrix somewhere
                          else is because, depending on the amino acid
                          types and labelling of the two residues,
                          the sequential peak-pattern differs and
                          therefor also the matching between expected
                          and real peaks.
      intraDict:          same as linkDict, only here a link objects are
                          stored that are not really links between two
                          spin-systems but rather holds important
                          information like the intra-residual peaks for
                          each spin-system that could be assigned to
                          this residue.
      atomsByName:        dict of {atomName:atom} in this residue.
      atomsByAtomSiteName: dict of {atomSite.name:atom}, where atomSite
                          is used in the graphs that descibe nmr
                          experiments in the ccpn data model.
      currentSpinSystemAssigned: can be used to store the current
                          spin-system assignment of the residue during
                          the monte-carlo run.
      userDefinedSolution: meant to be used temporarely to store a
                          spin-system assignment that is made by the
                          user while puzzling.
      solutions:          Usually multiple runs are performed. Here the
                          different spinSystems that were assigned in
                          the different runs can be stored.
    ...
    cdef void groupAtomsByAtomSite(self):
        ...
        if self.ccpCode == 'Phe' or self.ccpCode == 'Tyr':
            ...
            Calis = [atomsByName[name] for name in ['CA', 'CB']]
            atomsByAtomSiteName['Caro'] = [atomsByName[name]
                                           for name in ['CG', 'CD2', 'CE1']]
    ...
    cdef SpinSystemLink getFromLinkDict(self, SpinSystem spinSystem1,
                                        SpinSystem spinSystem2):
        ...
        hashCode = spinSystem1.spinSystemNumber * \
            10000 + spinSystem2.spinSystemNumber
    ...
    def __getstate__(self):
        ...
        return (self.chain, self.seqCode, self.ccpCode, self.atoms,
                self.solutions, self.linkDict, self.intraDict,
                self.previousResidue, self.nextResidue,
                self.userDefinedSolution)
    ...
    def connectToProject(self):
        ...
        self.ccpnResidue = self.chain.ccpnChain.findFirstResidue(
